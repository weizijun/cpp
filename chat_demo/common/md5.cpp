...
const static unsigned int ChunkSize = 4096;
...
*/
static void MD5Init(SMD5Context *context) /* context */
{
...
context.
*/
static void MD5Update(SMD5Context *context,   /* context */
...
void MD5(unsigned char acDigest[16], unsigned char* pInput, unsigned int dwInLen)
{
	...
	while(lastPos < dwInLen)
	{
		...
		{
			MD5Update(&mCtx, pInput + lastPos, dwInLen - lastPos);
			lastPos = dwInLen;
		}
		else
		{
			MD5Update(&mCtx, pInput + lastPos, ChunkSize);
			lastPos += ChunkSize;
		}
	}
    MD5Final(acDigest, &mCtx);
}

static std::string BufferToHex(const char *pBuffer, unsigned int BufferLength)
{
...
{
	unsigned char acDigest[16];
	MD5(acDigest, (unsigned char *)str, len);
	std::string strDigest = BufferToHex((char *)acDigest, 16);
	return strDigest;
}

static std::string BufferToBigHex(const char *pBuffer, unsigned int BufferLength)
{
	static unsigned char szHex[] = "0123456789ABCDEF";
...
std::string GetBigMD5(const char * str, int len)
{
	unsigned char acDigest[16];
	MD5(acDigest, (unsigned char *)str, len);
	std::string strDigest = BufferToBigHex((char *)acDigest, 16);
	return strDigest;
}
